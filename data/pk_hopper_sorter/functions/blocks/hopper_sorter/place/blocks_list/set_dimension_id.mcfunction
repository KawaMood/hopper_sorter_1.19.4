#> pk_hopper_sorter:blocks/hopper_sorter/place/blocks_list/set_dimension_id

# Default score
scoreboard players set $temp pk.dimension.id -99

# If placed in a vanilla dimension
execute if predicate pk_hopper_sorter:location/dimension/0 run scoreboard players set $temp pk.dimension.id 0
execute if predicate pk_hopper_sorter:location/dimension/-1 run scoreboard players set $temp pk.dimension.id -1
execute if predicate pk_hopper_sorter:location/dimension/1 run scoreboard players set $temp pk.dimension.id 1
...
execute store result storage pk.common:data Temp.HopperSorter.Dimension.id int 1 run scoreboard players get $temp pk.dimension.id
...
execute if score $temp pk.dimension.id matches -1..1 run function pk_hopper_sorter:base/uninstall/in_vanilla_dimension/start
# - If the custom block is in custom dimension
execute if score $temp pk.dimension.id matches 100.. run function pk_hopper_sorter:base/uninstall/in_custom_dimension/start